...
const CRITICAL_FILES = ['/', '/index.html', '/manifest.json', '/style.css'];
    ...
    Promise.allSettled([
      ...
      caches.open(IMAGE_CACHE).then((cache) => {
        ...
        return cache.addAll(IMAGE_FILES).catch((error) => {
          ...
          // 画像キャッシュ失敗は致命的ではない
          return Promise.allSettled(IMAGE_FILES.map((file) => cache.add(file)));
        });
      }),
      ...
      .then((results) => {
        ...
        if (failed.length > 0) {
          console.warn(
            '[SW] Some cache operations failed, but continuing:',
            failed
          );
        }
  ...
  for (const cacheName of cacheNames) {
    try {
      ...
      if (keys.length > maxSize) {
        ...
        console.log(
          `[SW] Optimized ${cacheName}: removed ${keysToDelete.length} entries`
        );
      }
    } catch (error) {
...
function recordPerformanceMetric(type, url, startTime) {
  ...
  const newAvg =
    (currentAvg * (self.performanceMetrics.totalRequests - 1) + duration) /
    self.performanceMetrics.totalRequests;
  ...
  if (duration > 3000) {
    console.warn(
      `[SW] Slow response: ${type} ${url} took ${duration.toFixed(2)}ms`
    );
  }
}
...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'SKIP_WAITING') {
    ...
    const stats = {
      ...self.performanceMetrics,
      cacheHitRate:
        self.performanceMetrics.totalRequests > 0
          ? (
              (self.performanceMetrics.cacheHits /
                self.performanceMetrics.totalRequests) *
              100
