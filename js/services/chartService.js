...
class ChartService {
    ...
    createOneRMChart(canvasId, data, options = {}) {
        try {
            ...
            // データの準備
            const labels = data.map(d => new Date(d.workout_date).toLocaleDateString('ja-JP', {
                month: 'short',
                day: 'numeric'
            }));
    ...
    createWeightChart(canvasId, data, options = {}) {
        try {
            ...
            const avgWeights = data.map(d =>
                d.weights.reduce((sum, w) => sum + w, 0) / d.weights.length
            );
    ...
    createGoalProgressChart(canvasId, goalsData, options = {}) {
        try {
            ...
            const labels = goalsData.map(g => g.description || `${g.goal_type}目標`);
            const progressValues = goalsData.map(g => g.progress_percentage);
            const colors = goalsData.map(g =>
                g.progress_percentage >= 100 ? this.defaultColors.success :
                    g.progress_percentage >= 75 ? this.defaultColors.warning :
