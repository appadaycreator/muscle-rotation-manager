...
class ReportService {
   ...
   */
    async generateProgressReportPDF(reportData, exerciseName) {
        try {
            ...
            pdf.text(`エクササイズ: ${exerciseName}`, pageWidth / 2, yPosition, {
                align: 'center'
            });
            yPosition += 20;
            ...
            if (reportData.trend) {
                ...
                const trendText =
          reportData.trend.direction === 'improving'
              ? '向上中'
              : reportData.trend.direction === 'declining'
                  ? '低下中'
                  : '安定';
                ...
                yPosition += 6;
                pdf.text(
                    `強度: ${reportData.trend.strength?.toFixed(2) || 'N/A'}`,
                    25,
                    yPosition
                );
                yPosition += 15;
   ...
   */
    generateWeeklySummary(weeklyData, exerciseName) {
        ...
        lastWeek.maxWeight > 0 && firstWeek.maxWeight > 0
            ? ((lastWeek.maxWeight - firstWeek.maxWeight) / firstWeek.maxWeight) *
            100
            : 0;

            const oneRMProgress =
        lastWeek.maxOneRM > 0 && firstWeek.maxOneRM > 0
            ? ((lastWeek.maxOneRM - firstWeek.maxOneRM) / firstWeek.maxOneRM) *
            100
            : 0;
            ...
            const avgSessions =
        sessionCounts.reduce((sum, count) => sum + count, 0) /
        sessionCounts.length;
            const variance =
        sessionCounts.reduce(
            (sum, count) => sum + Math.pow(count - avgSessions, 2),
            0
        ) / sessionCounts.length;
   ...
   */
    generateMonthlySummary(monthlyAnalysis, exerciseName) {
        try {
            ...
            const { stats, trend, totalSessions, dateRange, weeklyData } =
        monthlyAnalysis;

            // 週間一貫性の計算
            const weeklySessionCounts = weeklyData.map(
                (week) => week.sessions.length
            );
            const avgWeeklySessions =
        weeklySessionCounts.reduce((sum, count) => sum + count, 0) /
        ...
        try {
            ...
            if (avgWeeklySessions < 1) {
                recommendations.push(
                    'トレーニング頻度を増やすことを検討してください（週2-3回が理想的）'
                );
            } else if (avgWeeklySessions > 4) {
                recommendations.push(
                    'オーバートレーニングに注意し、適切な休息を取ってください'
                );
            }
            ...
            if (trend.direction === 'declining') {
                recommendations.push(
                    'パフォーマンスが低下しています。休息期間を設けるか、トレーニング強度を見直してください'
                );
                recommendations.push(
                    'フォームの確認や、栄養・睡眠の改善を検討してください'
                );
            } else if (trend.direction === 'stable' && stats.improvement < 2) {
                recommendations.push(
                    'プログレッシブ・オーバーロードの原則に従い、重量や回数を段階的に増やしてください'
                );
                recommendations.push(
                    '新しいエクササイズバリエーションを試してみてください'
                );
            }
            ...
            if (stats.improvement > 15) {
                recommendations.push(
                    '素晴らしい進歩です！現在のトレーニングプログラムを継続してください'
                );
            } else if (stats.improvement < 0) {
                recommendations.push(
                    'パフォーマンスが低下しています。トレーニングプログラムの見直しを検討してください'
                );
            }
            ...
            if (recommendations.length === 0) {
                recommendations.push(
                    '現在のトレーニングを継続し、定期的に進捗を確認してください'
                );
                recommendations.push(
                    '目標設定を行い、モチベーションを維持してください'
                );
            }
            ...
            if (workoutData.exercises && workoutData.exercises.length > 0) {
                ...
                workoutData.exercises.forEach((exercise) => {
                    ...
                    if (exercise.sets && exercise.reps && exercise.weight) {
                        pdf.text(
                            `  セット: ${exercise.sets}, 回数: ${exercise.reps}, 重量: ${exercise.weight}kg`,
                            30,
                            yPosition
                        );
                        yPosition += 6;
