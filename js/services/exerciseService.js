...
class ExerciseService {
    ...
    async searchExercises(searchTerm, filters = {}) {
        ...
        try {
            ...
            if (searchTerm && searchTerm.trim()) {
                ...
                query = query.or(`name_ja.ilike.%${term}%,name_en.ilike.%${term}%,search_keywords.ilike.%${term}%,description.ilike.%${term}%`);
            }
    ...
    async incrementUsageCount(exerciseId) {
        ...
        try {
            ...
            await supabaseService.client
                .rpc('increment_exercise_usage', { exercise_id: exerciseId });
        } catch (error) {
            // 使用回数の更新エラーは無視
    ...
    async uploadExerciseImage(file, exerciseId = null) {
        ...
        try {
            const bucketName = exerciseId ? 'exercise-images' : 'custom-exercise-media';
    ...
    async uploadExerciseVideo(file, exerciseId = null) {
        ...
        try {
            const bucketName = exerciseId ? 'exercise-videos' : 'custom-exercise-media';
