...
class AuthManager {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.matches('#login-btn, [data-action="login"]')) {
                ...
                if (!supabaseService.isAvailable()) {
                    showNotification(
                        'Supabaseが設定されていません。設定を確認してください。',
                        'error'
                    );
                    return;
                }
        ...
        try {
            ...
            if (currentUser) {
                ...
                if (userAvatar) {
                    ...
                    userAvatar.onerror = () => {
                        console.warn(
                            'Avatar image failed to load, using default:',
                            avatarUrl
                        );
   ...
   */
    async handleLogin(e) {
        ...
        // バリデーション実行
        const sanitizedData = globalFormValidator.validateAuthForm({
            email,
            password
        });
   ...
   */
    async handleSignup(e) {
      ...
      document.getElementById('privacy-agreement')?.checked;
