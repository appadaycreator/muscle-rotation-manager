...
class ConnectionStatusManager {
    ...
    createQueueIndicator() {
        ...
        this.queueIndicator.innerHTML = '📤 同期待ち: <span class="queue-count">0</span>件';
        this.queueIndicator.addEventListener('click', () => this.showQueueDetails());
    ...
    updateStatus(isOnline = null) {
        const currentStatus = isOnline !== null ? isOnline : supabaseService.getConnectionStatus();
    ...
    showSyncingStatus() {
        ...
        this.statusIndicator.innerHTML = '🔄 同期中 <span class="sync-indicator">⟳</span>';
    }
    ...
    updateQueueIndicator() {
        const queueLength = supabaseService.offlineQueue ? supabaseService.offlineQueue.length : 0;

        if (queueLength > 0) {
            this.queueIndicator.classList.remove('hidden');
            this.queueIndicator.querySelector('.queue-count').textContent = queueLength;
        } else {
    ...
    showQueueDetails() {
        ...
        const queueItems = supabaseService.offlineQueue.map((item, index) => {
    ...
    async forcSync() {
        ...
        try {
            ...
            this.showCustomAlert('同期に失敗しました。しばらく後に再試行してください。');
            this.updateStatus(false);
        }
    }
    ...
    showCustomAlert(message) {
        ...
        alertDiv.className = 'custom-alert fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
        alertDiv.innerHTML = `
