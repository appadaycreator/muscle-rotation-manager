   ...
   */
    ...
    // 認証状態が変更されたときにUIを更新
        supabaseService.onAuthStateChange(async (event, session) => {
            console.log('Auth state changed in settings page:', {
                event,
                session: !!session,
                user: !!session?.user
            });
    ...
    setupTooltips() {
        try {
            ...
            tooltipManager.addTooltip(
                '#display_name',
                '他のユーザーに表示される名前です。本名ではなく、ニックネームを入力してください。',
                {
                    position: 'right',
                    theme: 'primary',
                    animation: 'fadeIn',
            ...
            tooltipManager.addTooltip(
                '#email',
                'ログインに使用するメールアドレスです。変更する場合は、認証システムで更新してください。',
                {
                    position: 'right',
                    theme: 'warning',
                    animation: 'slide',
            ...
            tooltipManager.addTooltip(
                '#weight',
                '現在の体重を入力してください。カロリー計算やトレーニング強度の調整に使用されます。',
                {
                    position: 'right',
                    theme: 'primary',
                    animation: 'bounce',
            ...
            tooltipManager.addTooltip(
                '#height',
                '身長を入力してください。BMI計算や適切なトレーニング強度の算出に使用されます。',
                {
                    position: 'right',
                    theme: 'success',
                    animation: 'fadeIn',
            ...
            tooltipManager.addTooltip('#fitness-level-beginner', {
                content:
          '筋トレを始めたばかりの方。基本的なエクササイズから始めましょう。',
                position: 'top',
                theme: 'success',
                animation: 'bounce',
                arrow: true,
            ...
            tooltipManager.addTooltip('#fitness-level-intermediate', {
                content:
          '筋トレ経験が1年以上の方。中級レベルのエクササイズに挑戦できます。',
                position: 'top',
                theme: 'warning',
                animation: 'slide',
                arrow: true,
            ...
            tooltipManager.addTooltip('#fitness-level-advanced', {
                content:
          '筋トレ経験が豊富な方。高強度のエクササイズや上級テクニックに対応できます。',
                position: 'top',
                theme: 'error',
                animation: 'scale',
                arrow: true,
          ...
          '体脂肪を減らしたい方。有酸素運動と筋力トレーニングの組み合わせがおすすめです。',
            ...
            tooltipManager.addTooltip('#workout-frequency-2', {
                content:
          '週2回：初心者〜中級者向け。バランスの良いトレーニング頻度です。',
                position: 'top'
            });
            ...
            tooltipManager.addTooltip('#stress-level', {
                content:
          '現在のストレスレベル（1-10）。高いストレスは回復に影響するため、トレーニング強度を調整します。',
                position: 'top'
            });
            ...
            tooltipManager.addTooltip('#avatar-upload-btn', {
                content:
          'プロフィール画像をアップロードできます。JPG、PNG形式に対応（最大5MB）。',
                position: 'top'
            });
            ...
            tooltipManager.addTooltip('#export-data-btn', {
                content:
          'すべてのトレーニングデータをJSON形式でエクスポートします。バックアップとして使用できます。',
                position: 'top'
            });

            tooltipManager.addTooltip('#delete-data-btn', {
                content:
          '⚠️ 注意：すべてのトレーニングデータが削除されます。この操作は取り消せません。',
                position: 'top'
            });
   ...
   */
    collectAllSettings() {
        ...
        if (frequencyForm) {
            ...
            settings.workout_frequency =
        parseInt(formData.get('workout_frequency')) || 3;
            settings.preferred_workout_time = formData.get('preferred_workout_time');
            settings.preferred_workout_duration =
        parseInt(formData.get('preferred_workout_duration')) || 60;
        }
        ...
        if (recoveryForm) {
            ...
            settings.sleep_hours_per_night =
        parseFloat(formData.get('sleep_hours_per_night')) || 7.0;
   ...
   */
    async saveProfile(profileData) {
        ...
        if (supabaseService.isAvailable() && supabaseService.getCurrentUser()) {
            ...
            console.log(
                'Supabase not available or user not authenticated, using localStorage only'
            );
   ...
   */
    async handleAvatarUpload(e) {
        ...
        if (!allowedTypes.includes(file.type)) {
            showNotification(
                'JPEG、PNG、GIF、WebP形式の画像ファイルを選択してください',
                'error'
            );
            return;
        }
