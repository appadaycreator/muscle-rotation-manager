...
class AccessibilityManager {
  ...
  announce(message, priority = 'polite') {
    ...
    // 重複する通知を防ぐ
    if (this.announcements.includes(message)) {
      return;
    }
  ...
  getElementDescription(element) {
    ...
    return (
      descriptions[tagName] || element.textContent?.slice(0, 20) || 'コンテンツ'
    );
  }
  ...
  handleArrowNavigation(event) {
    ...
    if (
      currentElement.closest('.grid, .muscle-groups-grid, .exercise-presets')
    ) {
  ...
  handleGridNavigation(event) {
    const { key } = event;
    const grid = document.activeElement.closest(
      '.grid, .muscle-groups-grid, .exercise-presets'
    );
    const items = Array.from(
      grid.querySelectorAll('button, .clickable, [tabindex="0"]')
    );
  ...
  handleListNavigation(event) {
    ...
    const items = Array.from(
      list.querySelectorAll('[role="option"], li, .list-item')
    );
    const currentIndex = items.indexOf(document.activeElement);
  ...
  optimizeHeadingStructure() {
    ...
    headings.forEach((heading) => {
      ...
      if (level > currentLevel + 1) {
        console.warn(
          `見出し構造の問題: h${currentLevel}の後にh${level}があります`,
          heading
        );
      }
  ...
  toggleReducedMotionMode() {
    ...
    localStorage.setItem(
      'reducedMotionMode',
      this.reducedMotionMode.toString()
    );
  }
  ...
  enhanceSemanticMeaning() {
    ...
    document
      .querySelectorAll('.status, .notification, .alert')
      .forEach((element) => {
  ...
  calculateAccessibilityScore() {
    ...
    // キーボードナビゲーション
    maxScore += 20;
    if (this.getFocusableElements().length > 0) {
      score += 20;
    }
    ...
    console.log(
      `♿ アクセシビリティスコア: ${percentage}% (${score}/${maxScore})`
    );
  ...
  getAccessibilityStats() {
    return {
      ...
      ariaElements: document.querySelectorAll(
        '[aria-label], [aria-labelledby], [role]'
      ).length,
