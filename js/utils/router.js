    ...
    async loadPage(route) {
        try {
            ...
            try {
                ...
                console.warn(
                    `ページコンポーネント初期化に失敗しました (${route.component}):`,
                    componentError
                );
    ...
    // キャッシュをチェック
        ...
        try {
            ...
            if (response.status === 503) {
                console.warn(
                    `503 Service Unavailable for ${componentName}, retrying...`
                );
                await this.delay(1000); // 1秒待機
                const retryResponse = await fetch(`partials/${componentName}.html`);
                if (!retryResponse.ok) {
                    throw new Error(
                        `Failed to load partials/${componentName}.html after retry`
                    );
                }
   ...
   */
    async initializePageComponent(route) {
        try {
            ...
            console.error(
                `ページコンポーネント初期化エラー (${route.component}):`,
                error
            );
   ...
   */
    async reloadPageContent(componentName) {
        try {
            ...
            console.error(
                `ページコンテンツ再読み込みエラー (${componentName}):`,
                error
            );
        }
    }
