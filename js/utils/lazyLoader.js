...
class LazyLoader {
    ...
    async createPageLoadPromise(pageName, moduleKey) {
        ...
        try {
            ...
            const loadTime = performance.now() - startTime;
            console.error(`❌ 遅延ロード失敗: ${pageName} (${loadTime.toFixed(2)}ms)`, error);
            throw error;
        }
    }
    ...
    async loadServiceModule(serviceName) {
        ...
        const loadingPromise = this.createServiceLoadPromise(serviceName, moduleKey);
        this.loadingPromises.set(moduleKey, loadingPromise);
    ...
    async createServiceLoadPromise(serviceName, moduleKey) {
        ...
        try {
            ...
            const loadTime = performance.now() - startTime;
            console.log(`✅ サービス遅延ロード完了: ${serviceName} (${loadTime.toFixed(2)}ms)`);

            this.recordPerformanceMetric('service_load', serviceName, loadTime);
            return module;
        } catch (error) {
            const loadTime = performance.now() - startTime;
            console.error(`❌ サービス遅延ロード失敗: ${serviceName} (${loadTime.toFixed(2)}ms)`, error);
            throw error;
        }
    }
    ...
    loadImage(img) {
        ...
        img.onerror = () => {
            ...
            console.error(`❌ 画像読み込み失敗: ${img.src} (${loadTime.toFixed(2)}ms)`);
    ...
    async loadComponent(element) {
        ...
        try {
            ...
            const loadTime = performance.now() - startTime;
            console.log(`✅ コンポーネント遅延ロード完了: ${componentType} (${loadTime.toFixed(2)}ms)`);
            this.recordPerformanceMetric('component_load', componentType, loadTime);

        } catch (error) {
            const loadTime = performance.now() - startTime;
            console.error(`❌ コンポーネント遅延ロード失敗: ${componentType} (${loadTime.toFixed(2)}ms)`, error);
        }
    }
    ...
    checkPerformanceThreshold(type, duration) {
        ...
        if (threshold && duration > threshold) {
            console.warn(`⚠️ パフォーマンス警告: ${type} が閾値(${threshold}ms)を超えました: ${duration.toFixed(2)}ms`);
    ...
    getPerformanceStats() {
        ...
        // タイプ別に統計を計算
        ['page_load', 'service_load', 'image_load', 'component_load'].forEach(type => {
