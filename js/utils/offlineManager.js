...
class OfflineManager {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('workoutSessions')) {
                    ...
                    workoutStore.createIndex('syncStatus', 'sync_status', { unique: false });
                }
                ...
                if (!db.objectStoreNames.contains('trainingLogs')) {
                    ...
                    logsStore.createIndex('sessionId', 'workout_session_id', { unique: false });
    ...
    async saveWorkoutSession(sessionData) {
        const session = {
            ...sessionData,
            id: sessionData.id || `offline_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        try {
            ...
            console.log(`💾 ワークアウトセッション保存: ${session.id} (${session.sync_status})`);
            return session;
    ...
    async saveTrainingLogs(logs) {
        ...
        try {
            if (this.db) {
                ...
                // LocalStorageフォールバック
                await Promise.all(
                    processedLogs.map(log => this.saveToLocalStorage('trainingLogs', log))
                );
            }
            ...
            console.log(`💾 トレーニングログ保存: ${processedLogs.length}件 (${processedLogs[0]?.sync_status})`);
    ...
    async getWorkoutSessions(options = {}) {
        ...
        try {
            if (this.db) {
                return await this.getFromIndexedDB('workoutSessions', { limit, offset, userId });
            } else {
                return await this.getFromLocalStorage('workoutSessions', { limit, offset, userId });
            }
        } catch (error) {
    ...
    async processSyncQueue() {
            ...
            const batchPromises = batch.map(async (item, index) => {
                try {
                    ...
                    if (item.retryCount >= item.maxRetries) {
                        ...
                        console.warn(`⚠️ 同期諦め: ${item.type} (${item.retryCount}回試行)`);
                    }
                }
            });
        ...
        console.log(`✅ 同期完了: 成功 ${results.success}件, 失敗 ${results.failed}件`);
    }
