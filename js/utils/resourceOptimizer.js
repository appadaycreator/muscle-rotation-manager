...
class ResourceOptimizer {
    ...
    async initializeOptimizations() {
        try {
            ...
            console.warn('⚠️ リソース最適化の初期化で一部エラーが発生しました:', error.message);
            // 部分的な初期化失敗は許容する
        }
    }
    ...
    async checkWebPSupport() {
        ...
        return new Promise((resolve) => {
            ...
            webP.onload = webP.onerror = () => {
                ...
                console.log(`📷 WebP対応: ${this.webpSupported ? 'サポート' : '非サポート'}`);
    ...
    optimizeResponsiveImage(img) {
        if (img.srcset || img.sizes) {return;} // 既に設定済み
    ...
    identifyCriticalCSS() {
        ...
        try {
            localStorage.setItem('criticalCSS', JSON.stringify([...criticalSelectors]));
        } catch (error) {
        ...
        if (originalImport) {
            window.import = async (specifier) => {
                ...
                try {
                    ...
                    console.log(`📦 モジュール読み込み: ${specifier} (${loadTime.toFixed(2)}ms)`);
                    return module;
                } catch (error) {
    ...
    removeUnnecessaryPolyfills() {
        ...
        if (modernBrowser) {
            ...
            polyfillScripts.forEach(script => {
                if (script.src.includes('intersection-observer') ||
    ...
    monitorCoreWebVitals() {
        ...
        new PerformanceObserver((entryList) => {
            ...
            if (lastEntry.startTime > 2500) {
                console.warn('⚠️ LCP が遅いです:', lastEntry.startTime.toFixed(2), 'ms');
            }
        ...
        new PerformanceObserver((entryList) => {
            ...
            entries.forEach(entry => {
                ...
                if (duration > 1000) { // 1秒以上
                    console.warn('⚠️ 遅いリソース:', entry.name, duration.toFixed(2), 'ms');
                }
            });
    ...
    monitorMemoryUsage() {
        if ('memory' in performance) {
            setInterval(() => {
                ...
                console.log(`🧠 メモリ使用量: ${usedMB}MB / ${totalMB}MB (制限: ${limitMB}MB)`);

                // メモリ使用量が80%を超えた場合は警告
                const usageRatio = memory.usedJSHeapSize / memory.jsHeapSizeLimit;
                if (usageRatio > 0.8) {
                    console.warn('⚠️ メモリ使用量が高いです:', (usageRatio * 100).toFixed(1), '%');
                }
            }, 30000); // 30秒間隔
        }
    }
