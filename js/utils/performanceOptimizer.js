...
class PerformanceOptimizer {
    ...
    observeCoreWebVitals() {
        ...
        const fidObserver = new PerformanceObserver((list) => {
            ...
            entries.forEach(entry => {
                this.metrics.userInteractions.set('FID', entry.processingStart - entry.startTime);
    ...
    observeResourceTiming() {
        const resourceObserver = new PerformanceObserver((list) => {
            ...
            entries.forEach(entry => {
                ...
                // 遅いリソースを警告
                if (loadTime > 1000) {
                    console.warn('⚠️ 遅いリソース:', entry.name, loadTime.toFixed(2), 'ms');
                }
            });
        });
    ...
    setupCacheStrategy() {
        ...
        this.setCache = (key, value, ttl = 300000) => { // 5分デフォルト
    ...
    generateRecommendations(metrics) {
        ...
        if (lcp && lcp > 2500) {
            recommendations.push({
                type: 'performance',
                priority: 'high',
                message: 'LCPが2.5秒を超えています。画像最適化やクリティカルCSSの改善を検討してください。'
            });
        }
        ...
        if (fid && fid > 100) {
            recommendations.push({
                type: 'performance',
                priority: 'high',
                message: 'FIDが100msを超えています。JavaScriptの最適化を検討してください。'
            });
        }
        ...
        if (memory && memory.usage > 80) {
            recommendations.push({
                type: 'memory',
                priority: 'medium',
                message: 'メモリ使用率が80%を超えています。メモリリークの確認を推奨します。'
            });
        }
