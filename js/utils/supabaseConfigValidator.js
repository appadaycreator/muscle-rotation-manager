...
export function validateSupabaseConfig() {
    ...
    if (!SUPABASE_CONFIG.url) {
        ...
        validation.addSuggestion(
            'js/utils/constants.jsで実際のプロジェクトURLを設定してください'
        );
    ...
    if (!SUPABASE_CONFIG.key) {
        ...
        validation.addSuggestion(
            'js/utils/constants.jsで実際のAPI Keyを設定してください'
        );
        validation.isValid = false;
    } else if (!SUPABASE_CONFIG.key.startsWith('eyJ')) {
        validation.addError(
            'Supabase API KeyはJWTトークンである必要があります（eyJで始まる）'
        );
        validation.isValid = false;
    }
...
export function displayConfigInfo() {
    ...
    console.log(
        `   Key: ${SUPABASE_CONFIG.key ? `${SUPABASE_CONFIG.key.substring(0, 20)}...` : '未設定'}`
    );
...
if (typeof window !== 'undefined') {
    ...
    console.log(
        '🔧 設定検証を実行するには、コンソールで validateSupabaseConfig() を実行してください'
    );
}
