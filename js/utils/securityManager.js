...
class SecurityManager {
    ...
    setupClarityScript() {
        ...
        window.addEventListener('error', (event) => {
            if (event.filename && (event.filename.includes('clarity') || event.filename.includes('s3t9vl8h3v'))) {
        ...
        if (typeof window !== 'undefined') {
            ...
            window.clarity.push = window.clarity.push || function() {
    ...
    setupClarityFunctionProtection() {
        ...
        console.error = function(...args) {
            ...
            if (message.includes('a[c] is not a function') || message.includes('clarity')) {
    ...
    setupGlobalErrorHandling() {
        ...
        window.onerror = (message, source, lineno, colno, error) => {
            if (source && (source.includes('clarity') || source.includes('s3t9vl8h3v'))) {
                console.warn('Clarityスクリプトエラーを抑制しました:', message);
                return true; // エラーを抑制
            }
    ...
    setupAdvancedClarityProtection() {
        ...
        console.error = function(...args) {
            const message = args.join(' ');
            if (message.includes('a[c] is not a function') ||
                message.includes('clarity') ||
                message.includes('s3t9vl8h3v')) {
                console.warn('Clarityスクリプトエラーをフィルタリングしました:', message);
                return;
            }
            originalConsoleError.apply(console, args);
        };
    ...
    wrapScriptExecution() {
        ...
        window.setInterval = function(callback, delay, ...args) {
            ...
            // 文字列の場合は安全に実行
            return originalSetInterval.call(this, () => {
                try {
                    if (typeof callback === 'string') {
                        const safeFunction = new Function(`return ${callback}`);
                        safeFunction();
                    ...
                    console.warn('setInterval コールバックエラーを抑制しました:', error);
                }
            }, delay, ...args);
        };
    }
    ...
    setupComprehensiveErrorHandling() {
        ...
        window.addEventListener('error', (event) => {
            if (this.isClarityError(event)) {
                console.warn('Clarityスクリプトエラーを包括的に抑制しました:', event.error);
                event.preventDefault();
        ...
        document.addEventListener('error', (event) => {
            if (event.target.tagName === 'SCRIPT' && this.isClarityScript(event.target.src)) {
                console.warn('Clarityスクリプト読み込みエラーを抑制しました:', event.target.src);
                event.preventDefault();
                event.stopPropagation();
            }
        }, true);
    ...
    setupFinalErrorHandling() {
        ...
        window.onerror = (message, source, lineno, colno, error) => {
            // Clarityスクリプトのエラーを完全に抑制
            if (this.isClarityErrorMessage(message) || this.isClarityErrorSource(source)) {
        ...
        console.error = function(...args) {
            ...
            if (this.isClarityErrorMessage(message)) {
                console.warn('Clarityスクリプトエラーを最終的にフィルタリングしました:', message);
                return;
            }
            originalConsoleError.apply(console, args);
        }.bind(this);
        ...
        window.eval = function(code) {
            try {
                if (typeof code === 'string' && (
                    code.includes('clarity') ||
                    code.includes('s3t9vl8h3v')
                )) {
    ...
    setupAPIRateLimit() {
        ...
        window.fetch = async (url, options) => {
            ...
            if (!this.checkRateLimit(rateLimitKey, 10, 60000)) { // 1分間に10回
        ...
        this.validateData = (data, schema) => {
            for (const [key, rules] of Object.entries(schema)) {
                ...
                if (value !== undefined && rules.minLength && value.length < rules.minLength) {
                    throw new Error(`${key}は${rules.minLength}文字以上である必要があります`);
                }

                if (value !== undefined && rules.maxLength && value.length > rules.maxLength) {
                    throw new Error(`${key}は${rules.maxLength}文字以下である必要があります`);
                }
            }
        };
    }
    ...
    async checkSecurityHeaders(results) {
        ...
        results.recommendations.push({
            ...
            description: '適切なセキュリティヘッダーが設定されていることを確認してください'
        });
    }
