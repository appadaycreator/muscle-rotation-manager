...
export function showUserFriendlyError(errorInfo, options = {}) {
    ...
    const { message, description, solutions = [], severity = 'error' } = errorData;

    // エラー通知のHTML作成
    const errorId = `error-${Date.now()}`;
    const solutionsHtml = showSolutions && solutions.length > 0 ? `
        ...
        errorInfo.retry = async () => {
            if (errorInfo.retryCount < maxRetries) {
                ...
                try {
                    ...
                    console.warn(`❌ リトライ失敗 (${errorInfo.retryCount}/${maxRetries}):`, retryError);

                    if (errorInfo.retryCount >= maxRetries) {
                        const finalError = new Error(`最大リトライ回数(${maxRetries})に達しました`);
