...
class UIImprovementTestRunner {
  ...
  async setupTestEnvironment() {
    ...
    if (typeof document === 'undefined') {
      ...
      global.TouchEvent =
        dom.window.TouchEvent || class TouchEvent extends Event {};
    }
    ...
    const tests = [
      {
        name: 'ワークアウトウィザード初期化',
        test: async () => {
          const { MockWorkoutWizard } = await import(
            './unit/workout-wizard.test.js'
          );
          ...
          this.assert(
            ...
            'ワークアウトウィザードが正しく初期化される'
          );
          this.assert(
            wizard.selectedMuscleGroups.length === 0,
            '初期状態で筋肉部位が選択されていない'
          );
        },
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            wizard.selectedMuscleGroups.includes('chest'),
            '胸筋が選択される'
          );
          this.assert(
            wizard.selectedMuscleGroups.includes('back'),
            '背筋が選択される'
          );
        },
      },
    ...
    const tests = [
      {
        name: 'エラータイプ判定',
        test: async () => {
          const { MockErrorHandler } = await import(
            './unit/error-handling.test.js'
          );
          ...
          const validationType =
            errorHandler.determineErrorType(validationError);
          this.assert(
      ...
      },
      {
        ...
        test: async () => {
          ...
          const result = await errorHandler.executeWithRetry(retryFn, {
            maxRetries: 3,
          });
    ...
    const tests = [
      {
        name: 'タッチターゲット最適化',
        test: async () => {
          const { MockMobileOptimizationManager } = await import(
            './unit/mobile-accessibility.test.js'
          );
          ...
          this.assert(
            minHeight >= 44,
            `タッチターゲットが44px以上である (実際: ${minHeight}px)`
          );
        },
      },
      {
      ...
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            mobileOptimization.oneHandedMode === true,
            '片手操作モードが有効になる'
          );
          this.assert(
            document.body.classList.contains('one-handed-mode'),
            'CSSクラスが適用される'
          );
        },
      },
    ...
    const tests = [
      {
        ...
        test: async () => {
          const { MockAccessibilityManager } = await import(
            './unit/mobile-accessibility.test.js'
          );
          ...
          this.assert(
            accessibilityManager.liveRegion !== null,
            'ARIA Live Regionが作成される'
          );
          this.assert(
            accessibilityManager.liveRegion.getAttribute('aria-live') ===
              'polite',
            '適切なaria-live属性が設定される'
          );
        },
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            accessibilityManager.announcements.includes('テストメッセージ'),
            'アナウンスが記録される'
          );
          this.assert(
            accessibilityManager.liveRegion.textContent === 'テストメッセージ',
            'Live Regionにメッセージが設定される'
          );
        },
      },
      {
      ...
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            score >= 90,
            `アクセシビリティスコアが90点以上である (実際: ${score}点)`
          );
    ...
    const tests = [
      {
        name: '完全ワークアウトフロー',
        test: async () => {
          const { IntegratedWorkoutApp } = await import(
            './integration/ui-improvement-integration.test.js'
          );
    ...
    const tests = [
      {
        ...
        test: async () => {
          const operationReduction =
            this.performanceMetrics.get('operationReduction') || 0;
          this.assert(
            operationReduction >= 50,
            `操作数が50%以上削減される (実際: ${operationReduction}%)`
          );
        },
      },
      {
        ...
        test: async () => {
          const accessibilityScore =
            this.performanceMetrics.get('accessibilityScore') || 0;
          this.assert(
            accessibilityScore >= 90,
            `アクセシビリティスコアが90点以上 (実際: ${accessibilityScore}点)`
          );
        },
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            duration < 1000,
            `ワークアウトフローが1秒以内で完了する (実際: ${duration.toFixed(2)}ms)`
          );
        },
      },
      {
        ...
        test: async () => {
          ...
          this.assert(
            ...
            `メモリ使用量の増加が10MB以下 (実際: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB)`
          );
        },
      },
    ];
  ...
  async runTestGroup(groupName, tests) {
    ...
    for (const testCase of tests) {
      try {
        ...
        this.results.passed++;
        console.log(
          `  ✅ ${testCase.name} (${(endTime - startTime).toFixed(2)}ms)`
        );
      } catch (error) {
        this.results.failed++;
  ...
  generateReport() {
    ...
    console.log('\n📈 カバレッジ:');
    console.log(
      `  ステートメント: ${this.results.coverage.statements.toFixed(2)}%`
    );
    ...
    this.performanceMetrics.forEach((value, key) => {
      console.log(
        `  ${key}: ${value}${key.includes('Score') || key.includes('Reduction') ? '%' : ''}`
      );
    });
    ...
    const accessibilityScore =
      this.performanceMetrics.get('accessibilityScore') || 0;
    const errorRate =
      (this.results.failed / Math.max(this.results.total, 1)) * 100;

    console.log(
      `  操作数50%削減: ${operationReduction >= 50 ? '✅' : '❌'} (${operationReduction}%)`
    );
    console.log(
      `  アクセシビリティ90点以上: ${accessibilityScore >= 90 ? '✅' : '❌'} (${accessibilityScore}点)`
    );
    console.log(
      `  エラー率10%以下: ${errorRate <= 10 ? '✅' : '❌'} (${errorRate.toFixed(2)}%)`
    );
    console.log(
      `  テストカバレッジ98%以上: ${this.results.coverage.statements >= 98 ? '✅' : '❌'} (${this.results.coverage.statements.toFixed(2)}%)`
    );
    ...
    const results = {
      ...
      dodRequirements: {
        operationReduction: {
          ...
          passed:
            (this.performanceMetrics.get('operationReduction') || 0) >= 50,
        },
        accessibilityScore: {
          required: 90,
          actual: this.performanceMetrics.get('accessibilityScore') || 0,
          passed:
            (this.performanceMetrics.get('accessibilityScore') || 0) >= 90,
        },
        errorRate: {
          required: 10,
          actual: (this.results.failed / Math.max(this.results.total, 1)) * 100,
          passed:
            (this.results.failed / Math.max(this.results.total, 1)) * 100 <= 10,
        },
        testCoverage: {
          required: 98,
    ...
    if (typeof require !== 'undefined') {
      try {
        ...
        const resultsPath = path.join(
          __dirname,
          'results',
          'ui-improvement-test-results.json'
        );
