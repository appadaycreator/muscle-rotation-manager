...
class TestRunner {
    ...
    return {
      ...
      toHaveLength: (expected) => {
        if (actual.length !== expected) {
          throw new Error(
            `期待値: 長さ${expected}, 実際の値: 長さ${actual.length}`
          );
        }
      },
      toThrow: () => {
      ...
      toBeInstanceOf: (expected) => {
        if (!(actual instanceof expected)) {
          throw new Error(
            `期待値: ${expected.name}のインスタンス, 実際の値: ${actual.constructor.name}`
          );
        }
      },
      toHaveBeenCalled: () => {
      ...
      toHaveBeenCalledWith: (...expectedArgs) => {
        ...
        if (!found) {
          throw new Error(
            `期待値: ${JSON.stringify(expectedArgs)}で呼び出される, 実際の値: ${JSON.stringify(actual.calls)}`
          );
        }
      },
      ...
      toMatch: (pattern) => {
        ...
        if (!regex.test(actual)) {
          throw new Error(
            `期待値: パターン${pattern}にマッチ, 実際の値: ${actual}`
          );
        }
      },
      toBeDefined: () => {
  ...
  printResults(duration) {
    ...
    if (typeof require !== 'undefined') {
      ...
      fs.writeFileSync(
        'tests/results/unit-test-results.json',
