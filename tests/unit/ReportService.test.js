...
import {
  ReportService,
  reportService,
} from '../../js/services/reportService.js';
...
describe('ReportService', () => {
  ...
  describe('loadLibraries', () => {
    ...
    it('should handle library loading errors', async () => {
      ...
      jest
        .spyOn(service, 'loadLibraries')
        .mockRejectedValue(new Error('PDFライブラリの読み込みに失敗しました'));

      await expect(service.loadLibraries()).rejects.toThrow(
  ...
  describe('generateProgressReportPDF', () => {
    it.skip('should generate progress report PDF successfully', async () => {
      ...
      const result = await service.generateProgressReportPDF(
        reportData,
        'Bench Press'
      );
    ...
    it('should handle missing jsPDF library', async () => {
      ...
      await expect(
        service.generateProgressReportPDF({}, 'Test')
  ...
  describe('generateMonthlySummary', () => {
    it('should generate monthly summary successfully', () => {
      ...
      const result = service.generateMonthlySummary(
        monthlyAnalysis,
        'Bench Press'
      );
  ...
  describe('generateRecommendations', () => {
    it('should generate recommendations for low frequency', () => {
      ...
      expect(
        result.some((rec) => rec.includes('トレーニング頻度を増やす'))
      ).toBe(true);
    });
    ...
    it('should generate recommendations for high frequency', () => {
      ...
      expect(result.some((rec) => rec.includes('オーバートレーニング'))).toBe(
        true
      );
    });
    ...
    it('should generate recommendations for declining trend', () => {
      ...
      expect(result.some((rec) => rec.includes('パフォーマンスが低下'))).toBe(
        true
      );
    });
    ...
    it('should generate general recommendations when no specific conditions', () => {
      ...
      expect(
        result.some((rec) => rec.includes('現在のトレーニングを継続'))
      ).toBe(true);
    });
  });
  ...
  describe('exportToCSV', () => {
    ...
    it('should handle empty progress data', () => {
      expect(() => service.exportToCSV([], 'Bench Press')).toThrow(
        'エクスポートするデータがありません'
      );
    });

    it('should handle null progress data', () => {
      expect(() => service.exportToCSV(null, 'Bench Press')).toThrow(
        'エクスポートするデータがありません'
      );
    });
  });
  ...
  describe('generateWorkoutReportPDF', () => {
    ...
    it('should handle missing jsPDF library', async () => {
      ...
      await expect(service.generateWorkoutReportPDF({})).rejects.toThrow(
        'jsPDFライブラリが利用できません'
      );
    }, 15000);
  });
  ...
  describe('generateStatisticsReportPDF', () => {
    ...
    it('should handle missing jsPDF library', async () => {
      ...
      await expect(service.generateStatisticsReportPDF({})).rejects.toThrow(
        'jsPDFライブラリが利用できません'
      );
    }, 15000);
  });
  ...
  describe('exportToPDF', () => {
    ...
    it.skip('should handle missing libraries', async () => {
      ...
      await expect(service.exportToPDF({}, 'test.pdf')).rejects.toThrow(
        '必要なライブラリが利用できません'
      );
    }, 15000);
  });
  ...
  describe('integration', () => {
    it.skip('should handle complete report generation flow', async () => {
      ...
      const progressPDF = await service.generateProgressReportPDF(
        reportData,
        'Bench Press'
      );
      ...
      const weeklySummary = service.generateWeeklySummary(
        weeklyData,
        'Bench Press'
      );
      ...
      // Export to CSV
      const csvBlob = service.exportToCSV(
