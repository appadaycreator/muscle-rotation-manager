...
describe('仕様書駆動開発実践例: カレンダー機能', () => {
    ...
    describe('【SPEC準拠】月間表示機能', () => {
        test('指定月のカレンダーデータを生成できる', () => {
            const calendar = calendarManager.generateMonthlyCalendar(2024, 1, mockWorkoutData);
        ...
        test('週の開始日を設定できる', () => {
            // 日曜日開始
            const calendarSun = calendarManager.generateMonthlyCalendar(2024, 1, [], { weekStartsOn: 0 });
            expect(calendarSun.weekStartsOn).toBe(0);
            
            // 月曜日開始
            const calendarMon = calendarManager.generateMonthlyCalendar(2024, 1, [], { weekStartsOn: 1 });
    ...
    describe('【SPEC準拠】日別詳細機能', () => {
        test('特定日のワークアウト詳細を取得できる', () => {
            const dayDetail = calendarManager.getDayDetail('2024-01-15', mockWorkoutData);
            
            expect(dayDetail.date).toBe('2024-01-15');
            expect(dayDetail.workouts).toHaveLength(1);
            expect(dayDetail.workouts[0].muscleGroups).toEqual(['chest', 'shoulders']);
        ...
        test('複数のワークアウトがある日の詳細を取得できる', () => {
            ...
            const dayDetail = calendarManager.getDayDetail('2024-01-15', multipleWorkouts);
            
            expect(dayDetail.workouts).toHaveLength(2);
            expect(dayDetail.totalDuration).toBe(5400); // 60分 + 30分
            expect(dayDetail.uniqueMuscleGroups).toEqual(['chest', 'shoulders', 'abs']);
        });

        test('ワークアウトがない日の詳細を取得できる', () => {
            const dayDetail = calendarManager.getDayDetail('2024-01-01', mockWorkoutData);
    ...
    describe('【SPEC準拠】色分け表示機能', () => {
        ...
        test('日付に対応する色を取得できる', () => {
            const dayColors = calendarManager.getDayColors('2024-01-15', mockWorkoutData);
            
            expect(dayColors).toContain(calendarManager.getMuscleGroupColors().chest);
            expect(dayColors).toContain(calendarManager.getMuscleGroupColors().shoulders);
        ...
        test('ワークアウトがない日は中性色を返す', () => {
            const dayColors = calendarManager.getDayColors('2024-01-01', mockWorkoutData);
...
class CalendarManager {
    ...
    generateMonthlyCalendar(year, month, workoutData, options = {}) {
        ...
        for (let date = 1; date <= daysInMonth; date++) {
            ...
            const muscleGroups = [...new Set(
                dayWorkouts.flatMap(w => w.muscleGroups)
            )];
    ...
    blendColors(muscleGroups) {
        ...
        if (muscleGroups.length === 1) return this.muscleGroupColors[muscleGroups[0]];
