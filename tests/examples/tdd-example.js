...
describe('TDD実践例: ワークアウト統計計算機能', () => {
  ...
  test('部位別統計を正しく計算できる', () => {
    const statsByMuscleGroup =
      workoutStats.getStatsByMuscleGroup(sampleWorkouts);
...
class ImprovedWorkoutStatsCalculator {
  calculateTotalVolume(workouts) {
    ...
    return this._getAllSets(workouts).reduce(
  ...
  getStatsByMuscleGroup(workouts) {
    ...
    workouts.forEach((workout) => {
      workout.exercises.forEach((exercise) => {
        ...
        stats[group].totalVolume += exercise.sets.reduce(
